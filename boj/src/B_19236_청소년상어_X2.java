...
public class B_19236_청소년상어_X2 {
	...
	public static void main(String[] args) throws Exception {
		...
		moveShark(0, 0, map[0][0].num, map[0][0].dir, copyMap(map), copyFish(fish), "7");
		System.out.println(max);
	}
	
	static int max = 0;
	private static void moveShark(int x, int y, int sum, int dir, Fish[][] map, Point[] fish, String road) {
		map[x][y].dir = 8;
		...
		for(int i = 0; i < 4; i++) {
			...
			moveShark(tx, ty, sum + map[tx][ty].num, map[tx][ty].dir, copyMap(map), copyFish(fish), road + " " +  String.valueOf(map[tx][ty].num));
		}
	}
